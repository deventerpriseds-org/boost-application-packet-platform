...
// Before this, `jd_html` was never selected by any of the four call sites, so every figure and claim
...
test('the posting leads and is marked grounded', () => {
  const r = generationJd({ ...OPP, jd_html: '<p>You will own the roadmap for our hiring platform. 10+ years required.</p>' })
...
test('research context is kept but labelled as NOT from the posting', () => {
  const r = generationJd({ ...OPP, jd_html: '<p>Own the roadmap. 10+ years required.</p>' })
...
test('the alert digest is dropped once a real posting exists', () => {
  const r = generationJd({ ...OPP, jd_html: '<p>Own the roadmap for our platform.</p>' })
...
test('no posting: builds anyway, but is NEVER marked grounded', () => {
  const r = generationJd({ ...OPP, jd_html: null, jd_posting_raw: null })
  assert.equal(r.grounded, false)
  assert.ok(!/JOB POSTING/.test(r.jd))
  assert.match(r.jd, /VP Product at Acme/)
})

test('an alert digest in jd_posting_raw does not count as a posting', () => {
  const r = generationJd({ ...OPP, jd_html: null, jd_posting_raw: 'New LinkedIn alert: 12 jobs', why_surfaced: 'LinkedIn alert' })
  assert.equal(r.grounded, false)
})

test('the posting is bounded so a huge JD cannot blow the prompt', () => {
  const r = generationJd({ ...OPP, jd_html: '<p>' + 'own the roadmap. '.repeat(3000) + '</p>' })
  assert.equal(r.grounded, true)
