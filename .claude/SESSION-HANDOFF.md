...
- **Branch discipline (HARD RULE): never commit directly to `main`.** Develop on the session's designated feature branch; sync first (`git fetch origin && git merge origin/main`), push with `git push -u origin <branch>`, and fast-forward `main` from the branch at session end (`git merge --ff-only`).
