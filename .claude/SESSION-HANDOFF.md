...
- **`app/**` → `executive-engine-deploy.yml`** — triggers on push to **`main` only**, paths `app/**`. Single `concurrency` group keyed to the SWA (not the ref) — still required, so two rapid `main` pushes (or a `workflow_dispatch` overlapping a push) don't race into `"No matching Static Web App environment was found."` Don't delete that block.
- **`api/**` → `api-deploy.yml`** — triggers on push to **`main` only**, paths `api/**`.
- ⚠️ **Branch trap:** **a push to any branch other than `main` deploys NOTHING** — both workflows fire on `main` only. If you need to see a change live it must land on `main`. Confirm that before promising the owner a live change. (Before 2026-08-16 `app/**` also deployed to production from `claude/git-push-main-1zcqw5`; that trigger was removed. The branch still exists (fast-forwarded to `main`) — ref deletion is rejected by the CCR git proxy, so what closes the hole is the workflow file at that ref now reading `branches: [main]`, not the branch being gone.)
...
- ⚠️ There is no "the feature branch deploys too" shortcut any more — that trigger was removed on 2026-08-16. Every branch except `main` deploys nothing.
