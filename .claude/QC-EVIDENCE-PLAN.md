...
| **16** | **`closed_on_loop` grain (D-6) — NEEDS YOUR ANSWER.** It is one `int` on a per-OPPORTUNITY requirement row, but coverage is judged per-ARTIFACT across four artifacts with different merge fields (resume 7, cover 3). "Covered in the resume but not the cover letter" is the normal case. "Closed on loop 2" cannot express it. | **Recommend: closed = covered in at least one artifact, stated in the schema and surfaced WITH the artifact id.** The alternative is a per-artifact row, which is more honest but adds a table. Either is defensible; a bare int with no stated scope is not. |
| **17** | **Scoped regeneration does not exist (D-8) — SCOPE WARNING.** P3.1 says "re-run generation scoped to the open requirements only". `buildPackageForJD` is monolithic — call 2 consumes all of call 1, call 3 consumes both, `assemblePackage` merges whole payloads. **Nothing today can regenerate one merge field.** | **This is new capability, not wiring.** Flagging before building rather than after. Without it, pass 2 regenerates everything and loses content that was already correct — the exact failure P3.1 forbids. |
