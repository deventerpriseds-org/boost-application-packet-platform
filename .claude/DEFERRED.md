...
| D18 | **STILL LIVE — trigger has fired, work is not done.** Re-checked on `main` at `ed19230`: `theme.css:70` overrides `--surface-brand-subtle` under `.proto-dark` and there is NO `--surface-brand-default` override, so `.px-btn-accent`/`.px-btn-dark` (`:115`, `:117`) keep their light value in dark mode. P8.7 deployed and fixed a DIFFERENT P0.3-class defect (the `var(--temp-${k}-tint)` interpolations). | P0 is marked `done`; P8.7's report mentions a P0.3 fix; both are true and neither is this. | **Now** — needs a measured dark value, and P8.7's browser suite already measures contrast |
