...
const JSX_COLORS = {
  ...
  'var(--proto-red, #ef4444)': { grounds: DEFAULT_GROUNDS, why: 'RolesTitles.jsx:149 "Sign in to edit tiers" — real informational text, so it is measured. The fallback after the comma only applies if --proto-red is undefined, which it never is in this app; the token is what ships.' },
    'var(--triage-keep)': { grounds: ['paper'], why: 'Keep-triage button label. Now a TOKEN, not a literal - it measured 3.29 on the dark ground, which a hardcoded hex can never fix because it cannot adapt. Always on .px-btn, whose ground is paper.' },
    'var(--triage-maybe)': { grounds: ['paper'], why: 'Maybe-triage button label. Tokenised for the same reason: 4.10 light / 4.35 dark as a literal.' },
    'var(--gold)': { grounds: DEFAULT_GROUNDS, why: 'Gold. Two roles: the FavStar glyph (decorative, has aria-label) AND an 18px/800 count in RolesTitles - the count is text, so the stricter reading is measured. Tokenised: the literal was 2.89 on the light ground.' },
    'var(--linkedin)': { grounds: DEFAULT_GROUNDS, why: 'LinkedIn brand badge. Tokenised: 2.29 dark as a literal. Its own tint is composited over the ground first.', ownBg: 'var(--linkedin-tint)' },
