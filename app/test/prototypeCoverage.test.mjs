...
// A STATED TALLY MUST EQUAL ITS ROWS.
//
// The guard above is scoped to ABSENT rows on the stated reasoning that "PARTIAL and BUILT rot
// toward under-claiming, which is cheap". That holds for a ROW. It does not hold for a TALLY LINE,
// because the tally is what gets QUOTED - the headline parity number is built from them, and a
// stale tally under-reports finished work to the owner.
//
// Measured 2026-09-02: §4.1's line said BUILT 19 / PARTIAL 3 / ABSENT 2 while its rows said
// BUILT 20 / PARTIAL 2 / ABSENT 0, and §4.2's said BUILT 11 / PARTIAL 3 against rows saying
// BUILT 14 / PARTIAL 0. Rows had been re-verdicted and the tallies never followed - the exact
// "true when written, a lie by the next commit" failure this document warns about, inside the
// document itself. §4.3's line was accurate, which is why this is a drift check and not a rewrite.
//
// Mechanical, so it cannot cry wolf: it re-counts the verdict column and compares to the numbers
// the line states. It says nothing about whether a verdict is RIGHT - only that the summary and the
// rows agree.
test('H:coverage-tally-matches-rows: a stated section tally equals a recount of its rows', () => {
  const lines = readFileSync(DOC, 'utf8').split('\n')
  const heads = lines.map((l, i) => [i, l]).filter(([, l]) => /^## \d+\. SPEC §4\.\d/.test(l))
  const problems = []

  heads.forEach(([start], n) => {
    const end = n + 1 < heads.length ? heads[n + 1][0] : lines.length
    const tallyLine = lines.slice(start, end).find((l) => /^\*\*§4\.\d+ tally/.test(l))
    if (!tallyLine) return                       // a section without a stated tally claims nothing

    // Count the rows. The verdict is the 4th cell; take the FIRST verdict word in it, so
    // `**BUILT - CHANGED from ABSENT**` counts as BUILT rather than as both.
    const counted = {}
    for (const row of lines.slice(start, end)) {
      if (!row.startsWith('|')) continue
      const cells = row.replace(/^\||\|$/g, '').split('|').map((c) => c.trim())
      if (cells.length < 4) continue
      if (!/^\**`?4\.\d+-\d+`?\**$/.test(cells[0])) continue
      const m = cells[3].match(/\b(BUILT|PARTIAL|ABSENT|DELIBERATE|NOT-IN-PROTOTYPE|OUT-OF-SCOPE)\b/)
      if (m) counted[m[1]] = (counted[m[1]] || 0) + 1
    }

    // Compare only the verdicts the line actually STATES - a line may legitimately omit a category
    // it has none of, and demanding every category would fire on correct lines.
    for (const [, verdict, stated] of
      tallyLine.matchAll(/\b(BUILT|PARTIAL|ABSENT|DELIBERATE|NOT-IN-PROTOTYPE|OUT-OF-SCOPE)\*{0,2} \*\*(\d+)\*\*/g)) {
      const real = counted[verdict] || 0
      if (real !== Number(stated)) {
        problems.push(`${tallyLine.slice(0, 24)}… says ${verdict} ${stated}, rows say ${real}`)
      }
    }
  })

  assert.deepEqual(problems, [],
    'a stated tally disagrees with its own rows - the summary is quoted, so it must be recounted:\n  '
    + problems.join('\n  '))
})
