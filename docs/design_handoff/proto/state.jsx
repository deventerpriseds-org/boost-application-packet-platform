...
function AppProvider({ initial, children }) {
  ...
  // Apply dark-mode class + accent CSS var on root
  React.useEffect(() => {
...
function ToastTray() {
  ...
  return (
    <div style={{ position:'fixed', bottom:24, left:'50%', transform:'translateX(-50%)', zIndex:9999, display:'flex', flexDirection:'column', gap:6, pointerEvents:'none' }}>
      {toasts.map(t => (
        <div key={t.id} style={{
          background: PROTO.ink, color:'#fff', padding:'8px 14px',
          border:`2px solid ${PROTO.ink}`, fontFamily:"'Caveat', cursive",
          fontSize:15, fontWeight:600, animation:'toast-in 200ms ease-out',
