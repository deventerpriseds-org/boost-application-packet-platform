...
**§4.2 tally — 15 rows:** BUILT **14** · NOT-IN-PROTOTYPE **1**. *(Counts RECOUNTED from the rows 2026-09-02. The previous line was stale — rows had been re-verdicted and the tally never followed, so this section was UNDER-claiming. `H:coverage-tally-matches-rows` (`app/test/prototypeCoverage.test.mjs`).)*
...
**§4.3 tally — 14 rows:** BUILT **11** · DELIBERATE **2** · NOT-IN-PROTOTYPE **1**. *(Counts RECOUNTED from the rows 2026-09-02. The previous line was stale — rows had been re-verdicted and the tally never followed, so this section was UNDER-claiming. `H:coverage-tally-matches-rows` (`app/test/prototypeCoverage.test.mjs`).)*
...
**§4.4 tally — 33 rows:** BUILT **30** · DELIBERATE **3**. *(RECOUNTED from the rows 2026-09-02 — the stated line had gone stale as rows were re-verdicted, so this section was UNDER-claiming. `H:coverage-tally-matches-rows` (`app/test/prototypeCoverage.test.mjs`).)*
...
**§4.5 tally — 43 rows:** BUILT **36** · DELIBERATE **6** · NOT-IN-PROTOTYPE **1**. *(RECOUNTED from the rows 2026-09-02 — the stated line had gone stale as rows were re-verdicted, so this section was UNDER-claiming. `H:coverage-tally-matches-rows` (`app/test/prototypeCoverage.test.mjs`).)*
...
| 4.6-8 | Action: `Put back "<original>"` | `assets.jsx:70-74` | DELIBERATE | **CLOSED DELIBERATE 2026-09-03 — and the RECORDED REASON WAS WRONG, so both halves are stated.** (1) THE FEASIBILITY BLOCKER IS OVERTURNED. The 2026-09-02 entry concluded "ZERO rows whose `to_label` matches any keyword, so there is no join to render from". That measured LABEL EQUALITY. The real join is a FOREIGN KEY that already exists and is already written: `swap_decision.requirement_id -> requirement(id)` (`schema.ts:642`), populated by `writeSwaps` from `s.requirement_seq` (`appSwaps.ts:163-173`), with `model_keyword` a COLUMN ON `requirement` — a chip reaches its swap through its own requirement row, never through text. Measured live on `boost-pg-mcp-write`: 79 swap rows, 37 carry `requirement_id`, ALL 37 of those requirements have a `model_keyword`, and 19 of the 35 `swapped` rows join to one (`AI governance` displaced `Cloud Architecture` in `skills_1`; `architecture vision` displaced `Technology Strategy`; every one `driver: 'posting'`). The fuzzy-matching refusal is untouched — an FK is not a similarity score. (2) THE ACTUAL BLOCKER IS THAT THE ACTION HAS NOTHING TO CALL. `appSwaps.ts` registers exactly ONE route, `swapsGet`, GET-only, and no swap-revert mutation exists anywhere in `api/src`. `Put back "<original>"` is a MUTATION. This is the same principle already recorded for `4.11-7` and for `Ask why`'s absent `Undo this` sibling (`qcRail.js:792-795`): a disabled or dead control still asserts the capability exists. Re-open as a FEATURE — build `POST /app/packet/{id}/swaps/{swapId}/revert` — not as a parity gap. **CORRECTED 2026-09-03, same day: the reopening was itself wrong.**
That conclusion was reached from the button's LABEL, never its `onClick`. Reading the actual
prototype source (`docs/qc-evidence/qc/assets.jsx:70-74`): `onClick={() => onAsk(...)}` — it seeds
the field's ask box, the identical pattern already shipped for its two siblings on the same panel
(`Swap`, `Drop it`). No mutation route was ever needed, because the control it was compared against
does not use one. **BUILT 2026-09-03** as `keywordPutBackOption` (`assetBlocks.js`), wired at
`AssetBlocks.jsx` beside Drop/Swap, gated the same way they are (`canEdit && present`), and NOT
copying the prototype's sentence verbatim — its coverage-claim wording ("record the keyword as
uncovered rather than met") is something this app cannot honestly say, since `model_keyword` is
declared never scoreable. Guarded by `H:put-back-only-offered-for-a-real-displacement-that-is-present-and-editable`
and `H:put-back-names-both-terms-and-claims-no-coverage-state`, both mutation-proved FIRED. |
...
**§4.6 tally — 12 rows:** BUILT **8** · PARTIAL **0** · DELIBERATE **3** · NOT-IN-PROTOTYPE **1**. *(RECOUNTED from the rows 2026-09-02 — the stated line had gone stale as rows were re-verdicted, so this section was UNDER-claiming. `H:coverage-tally-matches-rows` (`app/test/prototypeCoverage.test.mjs`).)*
...
**§4.7 tally — 9 rows:** BUILT **9**. *(RECOUNTED from the rows 2026-09-02 — the stated line had gone stale as rows were re-verdicted, so this section was UNDER-claiming. `H:coverage-tally-matches-rows` (`app/test/prototypeCoverage.test.mjs`).)*
...
| 4.11-4 | Scope selector (This packet / This asset / My profile) | `assist.jsx` | **BUILT - CHANGED from ABSENT 2026-09-03** | Shipped as `assistantScopes` / `assistantSendBody` (`assistantPanel.js`), rendered `AssistantPanel.jsx` (`ASSISTANT_HOOKS.scopePick`/`scopeChip`), fed by PacketBuilder's existing `fieldFocus`. **TWO options, not three, and the difference is the point.** The prototype's own `send()` NEVER READS `scope` (`qc/assist.jsx:31-38`) - its three chips set local state that reaches nothing, the same `control that forgets` this repo refused at 4.5-38. So the options were derived from the write routes that exist, swept not assumed: every write is `app/artifact/{artifactId}/...`, so **This packet** has no route; `app/qc/facts/set` writes the profile but takes a STRUCTURED FACT, not an instruction, so **My profile** cannot carry an assistant ask. Both omitted rather than rendered inert (`no dead UI`). The two that remain are one parameter apart on the route that already ships - `artifactAiEdit` reads an optional `section`: set -> `pkg[section]` (one merge field), absent -> `art.content` (the whole asset) - so the selection changes what the model is handed, not just a label. Collapses to one option (no picker) when the caller has no field. Guards `H:scope-offers-only-what-actually-routes`, `H:scope-collapses-to-one-option-when-there-is-no-field`, `H:scope-sentence-states-what-that-scope-will-touch`, `H:scope-decides-the-section-the-handler-reads`, all mutation-proved. **SUPERSEDES the DELIBERATE close of the same day**, which read "a selector offering two options that cannot be honoured would assert capability the app does not have". That premise was correct about the prototype's THREE chips and wrong about two of them: `artifactAiEdit` already reads an optional `section`, so field-scope and asset-scope are both honoured today. Proven in the DOM, not argued - `run-assistant.mjs` claim 8: asset scope sends `{instruction}`, field scope sends `{instruction, section: 'ResumeSummary'}`. |
...
**§4.11 tally — 9 rows:** BUILT **6** · PARTIAL **0** · ABSENT **0** · DELIBERATE **3**. *(RECOUNTED from the rows 2026-09-02 — the stated line had gone stale as rows were re-verdicted, so this section was UNDER-claiming. `H:coverage-tally-matches-rows` (`app/test/prototypeCoverage.test.mjs`).)*
...
> # **176 of 176 prototype elements present (100%)**
>
> | | Count | Share of 175 |
> |---|---:|---:|
> | **BUILT** | **176** | **100.0%** |
> | **PARTIAL** | 0 | 0.0% |
> | **ABSENT** | **0** | **0.0%** |
> | *present (BUILT + PARTIAL)* | *175* | *100.0%* |
>
> **NO OPEN ROWS. Read what that does and does not mean.** It means every prototype element is either
> BUILT or carries a RECORDED decision for diverging — not that the app is finished, and not that
> every divergence was the prototype's loss. The DELIBERATE column is now 40-odd rows and each one
> cites its reason in code, in `actions.md`, or in an owner instruction.
>
> **The last row closed, `4.6-8`, is the one worth reading**, because its recorded blocker was WRONG
> and the correction runs both ways: the join it called impossible exists and is populated (an FK,
> measured live: 19 of 35 `swapped` rows reach a keyword), while the real blocker — no swap-revert
> mutation exists to call — was never written down. A row can be right about being blocked and wrong
> about why, and only the second half tells you what to build.
>
...
> and four assertions in `app/test/prototypeCoverage.test.mjs` now fail the suite if a stated tally
> ever disagrees with its own rows again. Criteria: `docs/qc-evidence/AC-tally-drift-guard.md`.
>
