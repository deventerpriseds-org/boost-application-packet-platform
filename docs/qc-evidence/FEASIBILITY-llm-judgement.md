...
| **is the model's reasoning HONEST** — `verifyReasoning` (`evidenceProposal.ts:340`) | named-token containment | **MODEL — added 2026-09-01** | **A6.** It withdraws the model's explanation for any named token the reasoning mentions and the quote does not literally contain. Measured: 2 of 10 Trinnex proposals withdrawn, and #20 wrongly — a BSc in *Information Systems* satisfies *"or related technical field"*, a clause `carries()` cannot read. **Keep the overclaim property, replace the test.** |
