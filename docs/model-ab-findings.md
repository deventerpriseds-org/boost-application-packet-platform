> **Imported reference — not produced in this repo.**
> Source: `deventerpriseds-org/huddle-extension-app` → `docs/model-ab-findings.md` @ `ef67eb5`,
> copied verbatim below on 2026-08-19. The A/B harness itself lives in that repo
> (`.claude/skills/test-agent-serverfn/scripts/model-ab.mjs` + `.github/workflows/model-ab.yml`);
> it runs in GitHub Actions because the CCR sandbox cannot reach OpenAI directly — the same
> constraint that applies here.
>
> **Why it is in this repo:** these are the only sourced GPT-5.6 prices we have, and this repo
> prices AI usage in `api/src/functions/tests/usageMeter.ts`. That `PRICES` map has **no
> `gpt-5.6-luna` entry**, so `costOf()` falls back to `gpt-4o-mini` ($0.15/$0.60 per 1M).
> Against the researched Luna rate ($0.20/$1.20) the artifact AI-edit path
> (`appPackets.ts:19`, `AI_EDIT_MODEL`) is under-reported **1.33× on input and 2× on output**.
> Not yet fixed — recorded here so the fix uses a sourced number rather than a guess.
>
> **Freshness:** prices were confirmed 2026-08-10 against OpenAI's pricing page reflecting the
> July-30-2026 cut. Re-confirm before relying on them for anything load-bearing.
